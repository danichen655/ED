...
public:
	...
	int count_interval(T const& lower, T const& upper) const {
        return count_intervalAux(lower, upper, _ra);
    }

    int count_intervalAux(T const& lower, T const& upper, Nodo* _raiz) const { //Log(n) ya que recorremos el arbol binario de forma recusiva en los nodos correspondientes

        if (_raiz == NULL) {
            return 0;
        }
        else {
            if (upper < _raiz->_elem) {
                return  count_intervalAux(lower, upper, _raiz->_iz);
            }
            else {
                if (lower > _raiz->_elem) {
                     return  count_intervalAux(lower, upper, _raiz->_dr);;
                }
                else {
                    if (upper == _raiz->_elem) {
                        return 1 + count_intervalAux(lower, upper, _raiz->_iz);
                    }
                    if (lower == _raiz->_elem) {
                        return  1 + count_intervalAux(lower, upper, _raiz->_dr);
                    }
                    else {
                        return 1 + count_intervalAux(lower, upper, _raiz->_iz) + count_intervalAux(lower, upper, _raiz->_dr);
                    }
                }
            }
        }
